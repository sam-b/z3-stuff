...
The hunt telling us to use z3 makes it clear that what we've been told is a series of constraints which apply to 20 letter variables which correspond to a number between 0 and 9 each. We need to us the hints to create constraints on symbolic variables which we can then use z3 to find valid values for.
...
Now I can run this script which I called 'generate.py' and it will output most of the code that is needed: 
...
I wrote a short stub of code in the file '15.py' - 
...
onto this I appended generates output, now I just needed to add the simultaneous equations as constraints and then parse the model generated by z3 if the constraints can be satisfied. 

I added the equations as constraints by just copy pasting chunks about into 'solv.add()' calls - I probably could have scripted this as well but it didn't seem worth the effort when most of them can just be pasted straight in.
...
Finally I wrote code to first see if the constraints were satisfiable and if so generate a model with the values for each letter in it. I then turn this into a python map and which I use to convert the letters in the original 'encrypted' string into the corresponding integers. I guessed that the letter pairs would be ascii values and it turned I was correct, so I cast each character pair to an int after conversion and then cast that into a char, all of which are appended together before being printed.
